...
import { config } from '../config/env.config.js';
  ...
  test('Flujo completo de calificación NL_TV', async ({ page }) => {
    ...
    await page.getByRole('option', { name: 'NETLIFE - TELEVENTAS' }).click();
    console.log('Seleccionado el módulo NETLIFE - TELEVENTAS');
    await page.locator('p-dropdown').filter({ hasText: 'Buscar agente' }).getByLabel('dropdown trigger').click();
    await page.getByRole('option', { name: 'Velez Chiluisa Ana' }).locator('div').first().click();
    ...
    await page.locator('(//input[@formcontrolname=\'valueKey\'])[6]').fill(config.callerId);
