...
import { config } from '../config/env.config.js';
  ...
  test('Flujo completo de calificación NL_L2', async ({ page }) => {
    ...
    await page.getByRole('option', { name: 'NETLIFE - L2' }).click();
    console.log('Seleccionado el módulo NETLIFE - L2');
    await page.locator('p-dropdown').filter({ hasText: 'Buscar agente' }).getByLabel('dropdown trigger').click();
    await page.getByRole('option', { name: 'Toapanta Vega Cristian' }).locator('div').first().click();
