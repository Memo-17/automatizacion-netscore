...
import { config } from '../config/env.config.js';
  ...
  test('Flujo completo de calificación NL_NA', async ({ page }) => {
    ...
    await page.getByRole('option', { name: 'NETLIFE - NA' }).click();
    console.log('Seleccionado el módulo NETLIFE - NA');
    await page.locator('p-dropdown').filter({ hasText: 'Buscar agente' }).getByLabel('dropdown trigger').click();
    await page.getByRole('option', { name: 'Lema Cajo Luis' }).locator('div').first().click();
